...
## Status (as built)
- DONE: repo bootstrap, engine import (`code-oss/` @ 2c314930), branding engine (`brand.ts`: 27 patches — identity + strings + JSON merges for `extensionsGallery` Open VSX + `onboardingThemes`), `gitcortex-theme` (Dark+Light), `gitcortex-tools` (Projects/Cloud views + deploy, marketplace delegated), `gitcortex-marketplace` (curated Open VSX entry), `gitcortex-ai` (orchestrator + 7 tools + pluggable transport + chat webview + native editor integration: explain/refactor/generate/fix/runTests + keybindings + context menus + **native AHP transport** pointing at the engine's own agent host via the local endpoint registry, with graceful fallback to the local planner, `tsc` clean, `test/smoke.ts` passes: 12 steps), `gitcortex/` product layer doc.
- Native AI migration: `transport.ahp.ts` discovers `<userDataPath>/agent-host/local-endpoint/entries/`, connects over WebSocket with `?tkn=`, performs AHP `initialize` (`protocolVersions: ["0.8.0"]`), `createSession`, `subscribe`, dispatches `chat/pendingMessageSet`. Default transport = `ahp`.
- Product layer separation documented in `gitcortex/README.md` + `docs/ARCHITECTURE.md` §4.4 (editor integration) §4.5 (Open VSX marketplace) + `docs/AI_AGENT.md` §3.3 (AHP).
- Auth: push requires `GITHUB_PERSONAL_ACCESS_TOKEN`.
- PENDING: full engine build validation (sandbox-blocked, no display), first release publish.
